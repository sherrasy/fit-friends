import { Controller, Get,Post, Patch, HttpStatus, Query,Body, Req, UseGuards, UseInterceptors } from '@nestjs/common';
import { API_TAG_NAME, OrdersError, OrdersMessages, OrdersPath } from './orders.constant';
import { ApiResponse, ApiTags } from '@nestjs/swagger';
import { JwtAuthGuard, fillObject } from '@backend/util/util-core';
import { CoachRoleInterceptor, UserRoleInterceptor } from '@backend/shared-interceptors';
...
export class OrdersController {
  constructor(
    private readonly workoutsOrderService: OrdersService,
  ) {}
  ...
  @UseInterceptors(CoachRoleInterceptor)
    public async showByCoach(@Req() { user }: RequestWithUserPayload, @Query() query:CoachOrderQuery) {
    const coachId = user.sub;
    const orders = await this.workoutsOrderService.findByCoachId(coachId, query);
  ...
  @Post()
  @UseInterceptors(UserRoleInterceptor)
  public async create(@Req() { user }: RequestWithUserPayload, @Body() dto: CreateOrderDto) {
    const userId = user.sub;
  ...
  @Patch()
  @UseInterceptors(UserRoleInterceptor)
  public async update(@Req() { user }: RequestWithUserPayload, @Body() dto: UpdateOrderAmountDto) {
    const userId = user.sub;
