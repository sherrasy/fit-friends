...
export class OrdersRepository {
  ...
  public async create(
    ...
    const data = {...item.toObject(), paymentOption: item.paymentOption as string};
  ...
  public async findByCoach(coachId: number, { limit, page, sortDirection, sortBy }:CoachOrderQuery): Promise<Workout[] | null> {
    const queryParams = {
      where: {
        AND: {
          coachId,
